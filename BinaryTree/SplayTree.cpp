...
void splay(Vertex*& root, Vertex* v) {
  if (v == NULL) return;
  while (v->parent != NULL) {
    if (v->parent->parent == NULL) {//only zig
      small_rotation(v);
      break;
    }
    big_rotation(v);
...
Vertex* find(Vertex*& root, int key) {
  Vertex* v = root;
  Vertex* last = root;
  Vertex* next = NULL;//key that bit larger than key,if key largest, return null
  while (v != NULL) {
...
void split(Vertex* root, int key, Vertex*& left, Vertex*& right) {
  right = find(root, key);
  splay(root, right);//most resently use node root , splay to top
  if (right == NULL) {//key is the largest
    left = root;
    return;
  }
  left = right->left;
  right->left = NULL;//split to left and right tree
  if (left != NULL) {
    left->parent = NULL;
  }
  update(left);
...
void erase(int x) {
  ...
  if (new_vertex == nullptr || new_vertex->key != x)
    return;//not find do nothing
  Vertex* left = NULL;
  Vertex* right = NULL;
  split(root, x, left, right);
  if (right != nullptr){
...
bool find(int x) {
  ...
  if (new_vertex == nullptr)
    return false;
  if (new_vertex->key != x)
    return false;//not find do nothing
  else
    return true;
}
